package com.investmentsite.isc.board.coin.service;

import com.investmentsite.isc.board.coin.dao.CoinBoardCommentRepository;
import com.investmentsite.isc.board.coin.dao.CoinBoardRepository;
...
public class CoinBoardService {
    ...
    public boolean boardPatch(BoardInput boardInput) {
        ...
        if (opCoinpa.isPresent()){
            //작성자가 맞는지 아닌지 체크
            if (boardInput.getWriter() == opCoinpa.get().getWriter()){
    ...
    public boolean commentPatch(BoardInput boardInput){
        ...
        if (opCommentpa.isPresent()){
            if (boardInput.getWriter() == opCommentpa.get().getWriter()){
    ...
    public boolean boardDelete(BoardInput boardInput){
        ...
        if (opCoinde.isPresent()){
            if (boardInput.getWriter() == opCoinde.get().getWriter()){
